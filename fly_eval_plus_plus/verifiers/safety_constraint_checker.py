...
class SafetyConstraintChecker(Verifier):
    ...
    def _calculate_airspeed_safety_score(self, avg_error, avg_rel_error, max_error, errors):
        ...
        """
        # 基础评分（基于平均误差） - 更严格的阈值
        if avg_error < 0.5:  # 误差 < 0.5 kt - 优秀
            base_score = 1.0
        elif avg_error < 1.0:  # 误差 < 1 kt - 良好
            base_score = 0.75
        elif avg_error < 2.0:  # 误差 < 2 kt - 一般
            base_score = 0.5
        elif avg_error < 5.0:  # 误差 < 5 kt - 较差
            base_score = 0.25
        else:  # 误差 >= 5 kt - 很差
            base_score = 0.0
    ...
    def _check_altitude_safety(self, predicted_data: Dict, ground_truth: Dict) -> List[EvidenceAtom]:
        ...
        try:
            ...
            # 高度误差评分 - 更严格的阈值
            if avg_error < 1.0:  # < 1 ft - 优秀
                score = 1.0
            elif avg_error < 3.0:  # < 3 ft - 良好
                score = 0.75
            elif avg_error < 5.0:  # < 5 ft - 一般
                score = 0.5
            elif avg_error < 10.0:  # < 10 ft - 较差
                score = 0.25
            else:  # >= 10 ft - 很差
                score = 0.0
    ...
    def _check_vertical_speed_safety(self, predicted_data: Dict, ground_truth: Dict) -> List[EvidenceAtom]:
        ...
        try:
            ...
            # 垂直速度误差评分 - 更严格的阈值
            if avg_error < 50:  # < 50 fpm - 优秀
                score = 1.0
            elif avg_error < 100:  # < 100 fpm - 良好
                score = 0.75
            elif avg_error < 200:  # < 200 fpm - 一般
                score = 0.5
            elif avg_error < 500:  # < 500 fpm - 较差
                score = 0.25
            else:  # >= 500 fpm - 很差
                score = 0.0
